...
targetName = "influenceGamma_d10_kappa100_expEigen"
...
#     KLtoTarget, ELBO, computationEffort, mean, cov = GaussianProjNGVIadjustedSampleSize(meanInit, covInit, eta, N, gamma, proj, alpha, beta, K, stepSizeSchedule, sampleSizeSchedule, target, N_ELBO)
...
# computationEffort = readdlm(directory*"/gamma025/computationEffort.txt")
# minBudget = computationEffort[length(computationEffort)]


# p_computational = plot(yscale=:log, xlims=(0,minBudget),xlabel="number of cumulated samples",ylabel="\$d_{A^*}(\\omega_*,\\omega_t)\$",labelfontsize = 13, tickfontsize = 13, legendfontsize = 13)
...
# methodName ="gamma025"
# methodLabel = "\$ \\eta = 10^{-2},\\,N_t=(t+1)^{1/4}\$"
...
# methodName ="gamma05"
# methodLabel = "\$ \\eta = 10^{-2},\\,N_t=(t+1)^{1/2}\$"
...
# methodName ="gamma15"
# methodLabel = "\$ \\eta = 10^{-2},\\,N_t=(t+1)^{3/2}\$"
...
# savefig(p_computational, directory*"/"*targetName*"_KLtoTarget_Computational.pdf")
# savefig(p_iterative, directory*"/"*targetName*"_KLtoTarget_Iteration.pdf")
